...
int feature_extraction_pipeline(int argc, char** argv) {
  ...
  string imagenet_binary_proto(argv[++arg_pos]);
  ...
  // First copy the imagenet trained layers
  feature_extraction_net->CopyTrainedLayersFrom(imagenet_binary_proto);

  // Now copy the layers trained by our model
