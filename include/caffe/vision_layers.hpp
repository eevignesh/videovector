 ...
 */
template <typename Dtype>
class SocialPoolingLayer : public Layer<Dtype> {
 public:
  explicit SocialPoolingLayer(const LayerParameter& param)
  ...
  virtual inline LayerParameter_LayerType type() const {
    return LayerParameter_LayerType_SOCIAL_POOLING;
  }
 ...
 protected:
  ...
  int feat_dim_;
  int pool_feat_size_;
  //Dtype pool_bin_size_;

  Blob<Dtype> diff_;
  Blob<Dtype> x_sel_;
  Blob<Dtype> y_sel_;
  Blob<Dtype> const_multiplier_;
  Blob<Dtype> temp_B_;
  Blob<Dtype> diff_feat_;
  Blob<Dtype> diff_feat_sig_;
  Blob<Dtype> eye_;
  Blob<Dtype> mean_added_data_;
  Blob<Dtype> f22_mat_;
  //Blob<Dtype> f11_mat_;
