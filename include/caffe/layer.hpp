...
class Layer {
 public:
  ...
  virtual void ToTruncatedProto(LayerParameter* param, bool write_diff = false, int truncate_num = 0);
  /**
...
// Serialize LayerParameter to protocol buffer
template <typename Dtype>
void Layer<Dtype>::ToTruncatedProto(LayerParameter* param, bool write_diff, int truncate_num) {
  param->Clear();
  param->CopyFrom(layer_param_);
  param->clear_blobs();
  for (int i = 0; i < blobs_.size(); ++i) {
    blobs_[i]->ToTruncatedProto(param->add_blobs(), write_diff, truncate_num);
  }
}
