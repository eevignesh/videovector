...
void LstmConditionalLayer<Dtype>::LayerSetUp(Blob<Dtype>* const& bottom) {

  const int num_output = this->layer_param_.inner_product_param().num_output();
  bias_term_ = this->layer_param_.inner_product_param().bias_term();
  clipping_threshold_ = this->layer_param_.lstm_param().clipping_threshold();
  O_ = this->layer_param_.lstm_param().final_output_size();
  encoder_mode_ = false;
  ...
  this->hidden_data_.Reshape(T_, B_, H_, 1);
  this->pre_tanh_out_.Reshape(T_, B_, O_, 1);
  ...
  caffe_set<Dtype>(T_*B_*H_, Dtype(0.), this->hidden_data_.mutable_cpu_data());
  caffe_set<Dtype>(T_*B_*O_, Dtype(0.), this->pre_tanh_out_.mutable_cpu_data());

  this->fdc_.Reshape(B_, H_, 1, 1);
  this->ig_.Reshape(B_, H_, 1, 1);
  this->temp_bh_.Reshape(B_, H_, 1, 1);

  this->param_propagate_down_.resize(7, true);

}

template <typename Dtype>
void LstmConditionalLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
...
//  B_ = bottom[0]->num() / T_;
  ...
  if (this->blobs_.size() > 0) {
    ...
    // Output to hidden weights (o2h)
    this->blobs_[2].reset(new Blob<Dtype>(1, 4, H_, O_));
    weight_filler->Fill(this->blobs_[2].get());

    // Hidden to output weights (h2o)
    this->blobs_[3].reset(new Blob<Dtype>(1, 1, O_, H_ ));
...
template <typename Dtype>
void LstmConditionalLayer<Dtype>::Reshape(const Blob<Dtype>* bottom_data,
...
template <typename Dtype>
void LstmConditionalLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      vector<Blob<Dtype>*>* top) {
  Reshape(bottom[0], bottom[1], (*top)[0]);
}

template <typename Dtype>
void LstmConditionalLayer<Dtype>::Forward_cpu(const Dtype* bottom_data,
  ...
  Dtype* top_data = this->hidden_data_.mutable_cpu_data();
  ...
  //Dtype* last_out_data = NULL;

  // Compute recurrent forward propagation
  for (int t = 0; t < T_; ++t) {

    Dtype* h_t = top_data + t*B_*H_;
    Dtype* output_t = top_output_data + t*B_*O_;
    ...
    if (t==0) {
      caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasTrans, B_, H_, O_, (Dtype)1.,
          first_input, weight_o2h, (Dtype)1., pre_i_t);
      caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasTrans, B_, H_, O_, (Dtype)1.,
          first_input, weight_o2h + H_*O_, (Dtype)1., pre_f_t);
      caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasTrans, B_, H_, O_, (Dtype)1.,
          first_input, weight_o2h + 2*H_*O_, (Dtype)1., pre_o_t);
      caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasTrans, B_, H_, O_, (Dtype)1.,
          first_input, weight_o2h + 3*H_*O_, (Dtype)1., pre_g_t);
    } else {
      caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasTrans, B_, H_, O_, (Dtype)1.,
          output_t - B_*O_, weight_o2h, (Dtype)1., pre_i_t);
      caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasTrans, B_, H_, O_, (Dtype)1.,
          output_t - B_*O_, weight_o2h + H_*O_, (Dtype)1., pre_f_t);
      caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasTrans, B_, H_, O_, (Dtype)1.,
          output_t - B_*O_, weight_o2h + 2*H_*O_, (Dtype)1., pre_o_t);
      caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasTrans, B_, H_, O_, (Dtype)1.,
          output_t - B_*O_, weight_o2h + 3*H_*O_, (Dtype)1., pre_g_t);
    }
    ...
    if (bias_term_) {
      caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, B_, H_, 1, (Dtype)1.,
          bias_multiplier_.cpu_data(), bias_o2h, (Dtype)1., pre_i_t);
      caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, B_, H_, 1, (Dtype)1.,
          bias_multiplier_.cpu_data(), bias_o2h + H_, (Dtype)1., pre_f_t);
      caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, B_, H_, 1, (Dtype)1.,
          bias_multiplier_.cpu_data(), bias_o2h + 2*H_, (Dtype)1., pre_o_t);
      caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, B_, H_, 1, (Dtype)1.,
          bias_multiplier_.cpu_data(), bias_o2h + 3*H_, (Dtype)1., pre_g_t);
   ...
   /*if ( t < 2) {
      LOG(ERROR) << " Computing output -------------------------------- ";
      for (int b = 0; b < 2; ++b) {
        LOG(ERROR)  << "B" << b << ":"
          << ", T" << t << ": "
          << hidden_data_.data_at(t, b, 0, 0) << ":"
          << hidden_data_.data_at(t, b, 0, 0);
      }
    }*/

    caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasTrans, B_, O_, H_, (Dtype)1.,
        h_t, weight_h2o, (Dtype)0., output_t);

    if (bias_term_) {
      caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, B_, O_, 1, (Dtype)1.,
          bias_multiplier_.cpu_data(), bias_h2o, (Dtype)1., output_t);
    }

//    caffe_copy<Dtype>(B_*O_, output_t, last_out_data);
  }
...
template <typename Dtype>
void LstmConditionalLayer<Dtype>::Backward_cpu(const Dtype* first_input,
  ...
  const Dtype* top_data = this->hidden_data_.cpu_data();
  ...
  Dtype* top_diff = this->hidden_data_.mutable_cpu_diff();
  ...
  caffe_set<Dtype>(T_*B_*H_, (Dtype)0., top_diff);

  for (int t = T_-1; t >= 0; --t) {

    Dtype* dh_t = top_diff + t*B_*H_;
    Dtype* doutput_t = top_output_diff + t*B_*O_;
    ...
    if (t < (T_-1)) {
      // TODO: update douptut_t
      // --------------------------------- Account for sequence continuation --------------
      caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, B_, O_, H_, (Dtype)1.,
        pre_di_t + B_*H_, weight_o2h, (Dtype)1., doutput_t);
      caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, B_, O_, H_, (Dtype)1.,
        pre_df_t + B_*H_, weight_o2h + H_*O_, (Dtype)1., doutput_t);
      caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, B_, O_, H_, (Dtype)1.,
        pre_do_t + B_*H_, weight_o2h + 2*H_*O_, (Dtype)1., doutput_t);
      caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, B_, O_, H_, (Dtype)1.,
        pre_dg_t + B_*H_, weight_o2h + 3*H_*O_, (Dtype)1., doutput_t);

    }

   // TODO: update dw_h2o, dbias_h2o 

    caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, B_, H_, O_, (Dtype)1.,
        doutput_t, weight_h2o, (Dtype)1., dh_t);
  ...
  if (bias_term_ && this->param_propagate_down_[4]) { 
    // Gradient w.r.t. bias
  ...
  // TODO: update dw_o2h, dbias_o2h
  if (this->param_propagate_down_[2]) {
    
    caffe_cpu_gemm<Dtype>(CblasTrans, CblasNoTrans, H_, O_, B_*(T_-1), (Dtype)1.,
      pre_gate_diff_i + B_*H_, top_output_data, (Dtype)0.0, weight_o2h_diff);
    caffe_cpu_gemm<Dtype>(CblasTrans, CblasNoTrans, H_, O_, B_*(T_-1), (Dtype)1.,
      pre_gate_diff_f + B_*H_, top_output_data, (Dtype)0.0, weight_o2h_diff + H_*O_);
    caffe_cpu_gemm<Dtype>(CblasTrans, CblasNoTrans, H_, O_, B_*(T_-1), (Dtype)1.,
      pre_gate_diff_o + B_*H_, top_output_data, (Dtype)0.0, weight_o2h_diff + 2*H_*O_);
    caffe_cpu_gemm<Dtype>(CblasTrans, CblasNoTrans, H_, O_, B_*(T_-1), (Dtype)1.,
      pre_gate_diff_g + B_*H_, top_output_data, (Dtype)0.0, weight_o2h_diff + 3*H_*O_);

    
    caffe_cpu_gemm<Dtype>(CblasTrans, CblasNoTrans, H_, O_, B_, (Dtype)1.,
      pre_gate_diff_i, first_input, (Dtype)1.0, weight_o2h_diff);
    caffe_cpu_gemm<Dtype>(CblasTrans, CblasNoTrans, H_, O_, B_, (Dtype)1.,
      pre_gate_diff_f, first_input, (Dtype)1.0, weight_o2h_diff + H_*O_);
    caffe_cpu_gemm<Dtype>(CblasTrans, CblasNoTrans, H_, O_, B_, (Dtype)1.,
      pre_gate_diff_o, first_input, (Dtype)1.0, weight_o2h_diff + 2*H_*O_);
    caffe_cpu_gemm<Dtype>(CblasTrans, CblasNoTrans, H_, O_, B_, (Dtype)1.,
      pre_gate_diff_g, first_input, (Dtype)1.0, weight_o2h_diff + 3*H_*O_);
  }
  
  if ( this->param_propagate_down_[5] && bias_term_) {

    caffe_cpu_gemm<Dtype>(CblasTrans, CblasNoTrans, H_, 1, B_*(T_-1), (Dtype)1.,
      pre_gate_diff_i + B_*H_, bias_multiplier_.cpu_data(), (Dtype)0.0, bias_o2h_diff);
    caffe_cpu_gemm<Dtype>(CblasTrans, CblasNoTrans, H_, 1, B_*(T_-1), (Dtype)1.,
      pre_gate_diff_f + B_*H_, bias_multiplier_.cpu_data(), (Dtype)0.0, bias_o2h_diff + H_);
    caffe_cpu_gemm<Dtype>(CblasTrans, CblasNoTrans, H_, 1, B_*(T_-1), (Dtype)1.,
      pre_gate_diff_o + B_*H_, bias_multiplier_.cpu_data(), (Dtype)0.0, bias_o2h_diff + 2*H_);
    caffe_cpu_gemm<Dtype>(CblasTrans, CblasNoTrans, H_, 1, B_*(T_-1), (Dtype)1.,
      pre_gate_diff_g + B_*H_, bias_multiplier_.cpu_data(), (Dtype)0.0, bias_o2h_diff + 3*H_);

    caffe_cpu_gemm<Dtype>(CblasTrans, CblasNoTrans, H_, 1, B_, (Dtype)1.,
      pre_gate_diff_i, bias_multiplier_.cpu_data(), (Dtype)1.0, bias_o2h_diff);
    caffe_cpu_gemm<Dtype>(CblasTrans, CblasNoTrans, H_, 1, B_, (Dtype)1.,
      pre_gate_diff_f, bias_multiplier_.cpu_data(), (Dtype)1.0, bias_o2h_diff + H_);
    caffe_cpu_gemm<Dtype>(CblasTrans, CblasNoTrans, H_, 1, B_, (Dtype)1.,
      pre_gate_diff_o, bias_multiplier_.cpu_data(), (Dtype)1.0, bias_o2h_diff + 2*H_);
    caffe_cpu_gemm<Dtype>(CblasTrans, CblasNoTrans, H_, 1, B_, (Dtype)1.,
      pre_gate_diff_g, bias_multiplier_.cpu_data(), (Dtype)1.0, bias_o2h_diff + 3*H_);
  }
  ...
  for (int b = 0; b < 2; ++b) {
    for (int t = 0; t < 2; ++t) {
      LOG(ERROR)  << "B" << b << ":"
        << ", T" << t << ": "
        << hidden_data_.data_at(t, b, 0, 0) << ":"
        << hidden_data_.data_at(t, b, 0, 1) << ":"
        << *(top_output_diff + t*B_*O_ + b*O_ + 0) << ":"
        << *(top_output_diff + t*B_*O_ + b*O_ + 1);
    }
  }*/

  if (this->param_propagate_down_[3]) {
    
    caffe_cpu_gemm<Dtype>(CblasTrans, CblasNoTrans, O_, H_, B_*T_, (Dtype)1.,
      top_output_diff, hidden_data_.cpu_data(), (Dtype)0.0, weight_h2o_diff);   
  }
  
  if ( this->param_propagate_down_[6] && bias_term_) {
    caffe_cpu_gemm<Dtype>(CblasTrans, CblasNoTrans, O_, 1, B_*T_, (Dtype)1.,
      top_output_diff, bias_multiplier_.cpu_data(), (Dtype)0.0, bias_h2o_diff);
  }
...
template <typename Dtype>
void LstmConditionalLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
    vector<Blob<Dtype>*>* top) {

  const Dtype* bottom_data = bottom[0]->cpu_data();
  const Dtype* bottom_cont = bottom[1]->cpu_data();
  const Dtype* first_input = bottom[2]->cpu_data();

  Dtype* top_data = (*top)[0]->mutable_cpu_data();
  const Dtype* weight_i = this->blobs_[0]->cpu_data();
  const Dtype* weight_h = this->blobs_[1]->cpu_data();
  const Dtype* weight_o2h = this->blobs_[2]->cpu_data();
  const Dtype* weight_h2o = this->blobs_[3]->cpu_data();

  if (bias_term_) {
   const Dtype* bias = this->blobs_[4]->cpu_data();
   const Dtype* bias_o2h = this->blobs_[5]->cpu_data();
   const Dtype* bias_h2o = this->blobs_[6]->cpu_data();

   Forward_cpu(bottom_data, first_input,
    bottom_cont, top_data,
    weight_i, weight_h,
    weight_h2o, weight_o2h,
    bias, bias_h2o, bias_o2h);

  } else {
   const Dtype* bias = NULL, *bias_o2h = NULL, *bias_h2o = NULL;
...
template <typename Dtype>
void LstmConditionalLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
    const vector<bool>& propagate_down,
    vector<Blob<Dtype>*>* bottom) {

  const Dtype* top_data = top[0]->cpu_data();

  /*LOG(ERROR) << " Checking top -------------------------------- ";
  for (int b = 0; b < 2; ++b) {
    for (int t = 0; t < 2; ++t) {
      LOG(ERROR)  << "B" << b << ":"
        << ", T" << t << ": "
        << hidden_data_.data_at(t, b, 0, 0) << ":"
        << hidden_data_.data_at(t, b, 1, 0) << ":"
        << top[0]->diff_at(t, b, 0, 0) << ":"
        << top[0]->diff_at(t, b, 1, 0);
    }
  }*/
  ...
  Dtype* weight_o2h_diff = this->blobs_[2]->mutable_cpu_diff();
  Dtype* weight_h2o_diff = this->blobs_[3]->mutable_cpu_diff();

  if (bias_term_) {
   Dtype* bias_diff = this->blobs_[4]->mutable_cpu_diff();
   Dtype* bias_o2h_diff = this->blobs_[5]->mutable_cpu_diff();
   Dtype* bias_h2o_diff = this->blobs_[6]->mutable_cpu_diff();

    Backward_cpu(first_input, top_data,
      top_diff, propagate_down[0], bottom_data,
      bottom_cont, bottom_mutable_diff_data,
      weight_i, weight_h, weight_h2o, weight_o2h,
      weight_i_diff, weight_h_diff, weight_h2o_diff, weight_o2h_diff,
      bias_diff, bias_h2o_diff, bias_o2h_diff);

  } else {
    Dtype* bias_diff = NULL, *bias_h2o_diff = NULL, *bias_o2h_diff = NULL; 
    Backward_cpu(first_input, top_data,
      top_diff, propagate_down[0], bottom_data,
      bottom_cont, bottom_mutable_diff_data,
      weight_i, weight_h, weight_h2o, weight_o2h,
...
#ifdef CPU_ONLY
STUB_GPU(LstmConditionalLayer);
#endif

INSTANTIATE_CLASS(LstmConditionalLayer);
