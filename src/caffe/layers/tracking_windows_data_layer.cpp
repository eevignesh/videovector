...
TrackingWindowsDataLayer<Dtype>::~TrackingWindowsDataLayer<Dtype>() {
...
template <typename Dtype>
void TrackingWindowsDataLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
void TrackingWindowsDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
      vector<Blob<Dtype>*>* top) {
  ...
  (*top)[1]->Reshape(temporal_observed_size_,
        batch_size_, 1, 1);
  (*top)[2]->Reshape(temporal_predicted_size_,
        batch_size_, feature_size_predicted_, 1);
  (*top)[3]->Reshape(temporal_predicted_size_,
        batch_size_, 1, 1);

  if (this->output_labels_) {
    (*top)[4]->Reshape(temporal_predicted_size_,
        batch_size_, 2, 1);
  ...
  this->prefetch_data_0_.Reshape(temporal_observed_size_,
      batch_size_, feature_size_observed_, 1);
  this->prefetch_data_1_.Reshape(temporal_observed_size_,
      batch_size_, 1, 1);
  this->prefetch_data_2_.Reshape(temporal_predicted_size_,
      batch_size_, feature_size_predicted_, 1);
  this->prefetch_data_3_.Reshape(temporal_predicted_size_,
      batch_size_, 1, 1);
...
void TrackingWindowsDataLayer<Dtype>::InternalThreadEntry() {
  ...
  for (int i = 0; i < batch_size_; ++i) {
    ...
    switch (this->layer_param_.tracking_windows_data_param().backend()) {
      case TrackingWindowsDataParameter_DB_LEVELDB:
        iter_->Next();
        if (!iter_->Valid() || (num_positions_==0 && max_number_positions_ > 0)) {
      ...
      case TrackingWindowsDataParameter_DB_LMDB:
        if (mdb_cursor_get(mdb_cursor_, &mdb_key_,
            ...
            || (num_positions_==0 && max_number_positions_ > 0)
            ) {
          // We have reached the end. Restart from the first.
          LOG(INFO) << "Restarting data prefetching from start.";
    ...
    /*Dtype mean_x = (Dtype)0.;
    Dtype mean_y = (Dtype)0.;

    for (int t = 0; t < temporal_observed_size_; ++t) {
      mean_x += (Dtype)track_position.x(t);
      mean_x += (Dtype)track_position.y(t);
    }

    mean_x /= (Dtype)(temporal_observed_size_);
    mean_y /= (Dtype)(temporal_observed_size_);*/
    ...
    for (int t = 0; t < temporal_observed_size_; ++t) {
      ...
      *(top_data_0 + this->prefetch_data_0_.offset(t, i, 1, 0)) = (Dtype)track_position.y(t)*this->layer_param_.tracking_windows_data_param().track_scale()- mean_y;
      ...
      //caffe_set<Dtype>(1, (Dtype)track_position.y(t),
      //   top_data_0 + this->prefetch_data_0_.offset(t, i, 1, 0));
    ...
    for (int t = 0; t < temporal_predicted_size_; ++t) {
      ...
      /*LOG(INFO) << prev_track_id_ << "/" << prev_tracking_window_.track_positions_size() << " -----------> "
        << "t: " << t << " i: " << i
        << " : " << track_position.x(t + temporal_observed_size_)
        << " : " << track_position.y(t + temporal_observed_size_);*/


      /*caffe_set<Dtype>(1, (Dtype)track_position.x(t + temporal_observed_size_),
         top_label + this->prefetch_label_.offset(t, i, 0, 0));
      caffe_set<Dtype>(1, (Dtype)track_position.y(t + temporal_observed_size_),
         top_label + this->prefetch_label_.offset(t, i, 1, 0));*/
      if (use_static_scene_) {
  ...
  /*for (int i = 0; i < 4; ++i) {
    for (int t = 0; t < 20; ++t) {
      LOG(INFO) << "t: " << t << " i: " << i
        << " : " << this->prefetch_label_.data_at(t, i, 0, 0)
        << " : " << this->prefetch_label_.data_at(t, i, 1, 0);
    }
  }*/

  caffe_set<Dtype>(batch_size_*temporal_observed_size_, (Dtype)1.0, top_data_1);
  caffe_set<Dtype>(batch_size_, (Dtype)0.0, top_data_1);
  caffe_set<Dtype>(batch_size_*temporal_predicted_size_, (Dtype)1.0, top_data_3);

}

template <typename Dtype>
void TrackingWindowsDataLayer<Dtype>::CreatePrefetchThread() {
  this->phase_ = Caffe::phase();
  CHECK(StartInternalThread()) << "Thread execution failed";
}

template <typename Dtype>
void TrackingWindowsDataLayer<Dtype>::JoinPrefetchThread() {
  CHECK(WaitForInternalThreadToExit()) << "Thread joining failed";
}

template <typename Dtype>
void TrackingWindowsDataLayer<Dtype>::Forward_cpu(
...
#ifdef CPU_ONLY
STUB_GPU_FORWARD(TrackingWindowsDataLayer, Forward);
#endif

INSTANTIATE_CLASS(TrackingWindowsDataLayer);
