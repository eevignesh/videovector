...
void LstmConditionalLayer<Dtype>::Forward_gpu(const Dtype* bottom_data,
  ...
  Dtype* top_data = this->hidden_data_.mutable_gpu_data();
  ...
  Dtype* pre_tanh_out_data = pre_tanh_out_.mutable_gpu_data();
  ...
  for (int t = 0; t < T_; ++t) {
    ...
    Dtype* pre_tanh_out_t = pre_tanh_out_data + t*B_*O_;
    ...
    if (t==0) {
      // TODO: INCLUDE fisr_input
      /*caffe_gpu_gemm<Dtype>(CblasNoTrans, CblasTrans, B_, H_, O_, (Dtype)1.,
    ...
    if (bias_term_) {
      caffe_gpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, B_, H_, 1, (Dtype)1.,
          bias_multiplier_.gpu_data(), bias_o2h, (Dtype)1., pre_i_t);
      caffe_gpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, B_, H_, 1, (Dtype)1.,
          bias_multiplier_.gpu_data(), bias_o2h + H_, (Dtype)1., pre_f_t);
      caffe_gpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, B_, H_, 1, (Dtype)1.,
          bias_multiplier_.gpu_data(), bias_o2h + 2*H_, (Dtype)1., pre_o_t);
      caffe_gpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, B_, H_, 1, (Dtype)1.,
          bias_multiplier_.gpu_data(), bias_o2h + 3*H_, (Dtype)1., pre_g_t);
   ...
   /*if ( t < 2) {
    ...
    caffe_gpu_gemm<Dtype>(CblasNoTrans, CblasTrans, B_, O_, H_, (Dtype)1.,
        h_t, weight_h2o, (Dtype)0., pre_tanh_out_t);

    if (bias_term_) {
      caffe_gpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, B_, O_, 1, (Dtype)1.,
          bias_multiplier_.gpu_data(), bias_h2o, (Dtype)1., pre_tanh_out_t);
    }

    //tanh_->Forward_gpu(B_*O_, pre_tanh_out_t, output_t);

    caffe_copy<Dtype>(B_*O_, pre_tanh_out_t, output_t);
...
template <typename Dtype>
void LstmConditionalLayer<Dtype>::Backward_gpu(const Dtype* first_input,
    const Dtype* top_output_data, Dtype* top_output_diff,
  ...
  const Dtype* top_data = this->hidden_data_.gpu_data();
  ...
  Dtype* top_diff = this->hidden_data_.mutable_gpu_diff();
  ...
  Dtype* dpre_tanh_out_data = pre_tanh_out_.mutable_gpu_diff();
  //const Dtype* pre_tanh_out_data = pre_tanh_out_.gpu_data();

  caffe_gpu_set<Dtype>(4*H_*H_, (Dtype)0., weight_h_diff);
  caffe_gpu_set<Dtype>(T_*B_*H_, (Dtype)0., top_diff);
  ...
  for (int t = T_-1; t >= 0; --t) {
    ...
    Dtype* dpre_tanh_out_t = dpre_tanh_out_data + t*B_*O_;
    //const Dtype* pre_tanh_out_t = pre_tanh_out_data + t*B_*O_;
    ...
    if (t < (T_-1)) {
      ...
      caffe_gpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, B_, O_, H_, (Dtype)1.,
   ...
   // TODO: update dw_h2o, dbias_h2o 
    ...
    //tanh_->Backward_gpu(B_*O_, top_data + t*B_*O_, doutput_t, dpre_tanh_out_t);
    
    caffe_copy<Dtype>(B_*O_, doutput_t, dpre_tanh_out_t);
    caffe_gpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, B_, H_, O_, (Dtype)1.,
        dpre_tanh_out_t, weight_h2o, (Dtype)1., dh_t);


    // Output gate : tanh(c(t)) * h_diff(t)
    caffe_gpu_mul<Dtype>(B_*H_, tanh_c_t, dh_t, do_t);
  ...
  if (this->param_propagate_down_[2]) {
    ...
    caffe_gpu_gemm<Dtype>(CblasTrans, CblasNoTrans, H_, O_, B_*(T_-1), (Dtype)1.,
  ...
  // TODO: INCLUDE fisr_input
    /*caffe_gpu_gemm<Dtype>(CblasTrans, CblasNoTrans, H_, O_, B_, (Dtype)1.,
  ...
  if ( this->param_propagate_down_[5] && bias_term_) {
    ...
    caffe_gpu_gemm<Dtype>(CblasTrans, CblasNoTrans, H_, 1, B_*(T_-1), (Dtype)1.,
      pre_gate_diff_i + B_*H_, bias_multiplier_.gpu_data(), (Dtype)0.0, bias_o2h_diff);
    caffe_gpu_gemm<Dtype>(CblasTrans, CblasNoTrans, H_, 1, B_*(T_-1), (Dtype)1.,
      pre_gate_diff_f + B_*H_, bias_multiplier_.gpu_data(), (Dtype)0.0, bias_o2h_diff + H_);
    caffe_gpu_gemm<Dtype>(CblasTrans, CblasNoTrans, H_, 1, B_*(T_-1), (Dtype)1.,
      pre_gate_diff_o + B_*H_, bias_multiplier_.gpu_data(), (Dtype)0.0, bias_o2h_diff + 2*H_);
    caffe_gpu_gemm<Dtype>(CblasTrans, CblasNoTrans, H_, 1, B_*(T_-1), (Dtype)1.,
      pre_gate_diff_g + B_*H_, bias_multiplier_.gpu_data(), (Dtype)0.0, bias_o2h_diff + 3*H_);

    caffe_gpu_gemm<Dtype>(CblasTrans, CblasNoTrans, H_, 1, B_, (Dtype)1.,
      pre_gate_diff_i, bias_multiplier_.gpu_data(), (Dtype)1.0, bias_o2h_diff);
    caffe_gpu_gemm<Dtype>(CblasTrans, CblasNoTrans, H_, 1, B_, (Dtype)1.,
      pre_gate_diff_f, bias_multiplier_.gpu_data(), (Dtype)1.0, bias_o2h_diff + H_);
    caffe_gpu_gemm<Dtype>(CblasTrans, CblasNoTrans, H_, 1, B_, (Dtype)1.,
      pre_gate_diff_o, bias_multiplier_.gpu_data(), (Dtype)1.0, bias_o2h_diff + 2*H_);
    caffe_gpu_gemm<Dtype>(CblasTrans, CblasNoTrans, H_, 1, B_, (Dtype)1.,
      pre_gate_diff_g, bias_multiplier_.gpu_data(), (Dtype)1.0, bias_o2h_diff + 3*H_);
  }
  ...
  if (this->param_propagate_down_[3]) {
    ...
    caffe_gpu_gemm<Dtype>(CblasTrans, CblasNoTrans, O_, H_, B_*T_, (Dtype)1.,
      top_output_diff, hidden_data_.gpu_data(), (Dtype)0.0, weight_h2o_diff);   
  }
  
  if ( this->param_propagate_down_[6] && bias_term_) {
    caffe_gpu_gemm<Dtype>(CblasTrans, CblasNoTrans, O_, 1, B_*T_, (Dtype)1.,
      dpre_tanh_out_data, bias_multiplier_.gpu_data(), (Dtype)0.0, bias_h2o_diff);
  }
...
template <typename Dtype>
void LstmConditionalLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
    vector<Blob<Dtype>*>* top) {

  const Dtype* bottom_data = bottom[0]->gpu_data();
  const Dtype* bottom_cont = bottom[1]->gpu_data();
  const Dtype* first_input = bottom[2]->gpu_data();

  Dtype* top_data = (*top)[0]->mutable_gpu_data();
  const Dtype* weight_i = this->blobs_[0]->gpu_data();
  const Dtype* weight_h = this->blobs_[1]->gpu_data();
  const Dtype* weight_o2h = this->blobs_[2]->gpu_data();
  const Dtype* weight_h2o = this->blobs_[3]->gpu_data();

  if (bias_term_) {
   const Dtype* bias = this->blobs_[4]->gpu_data();
   const Dtype* bias_o2h = this->blobs_[5]->gpu_data();
   const Dtype* bias_h2o = this->blobs_[6]->gpu_data();

   Forward_gpu(bottom_data, first_input,
...
template <typename Dtype>
void LstmConditionalLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  Dtype* weight_o2h_diff = this->blobs_[2]->mutable_gpu_diff();
  Dtype* weight_h2o_diff = this->blobs_[3]->mutable_gpu_diff();

  if (bias_term_) {
   Dtype* bias_diff = this->blobs_[4]->mutable_gpu_diff();
   Dtype* bias_o2h_diff = this->blobs_[5]->mutable_gpu_diff();
   Dtype* bias_h2o_diff = this->blobs_[6]->mutable_gpu_diff();

    Backward_gpu(first_input, top_data,
