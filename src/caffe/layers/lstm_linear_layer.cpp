...
void LstmLinearLayer<Dtype>::LayerSetUp(Blob<Dtype>* const& bottom) {
...
template <typename Dtype>
void LstmLinearLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
void LstmLinearLayer<Dtype>::Reshape(const Blob<Dtype>* bottom_data,
...
template <typename Dtype>
void LstmLinearLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      vector<Blob<Dtype>*>* top) {
  Reshape(bottom[0], bottom[1], (*top)[0]);
}

template <typename Dtype>
void LstmLinearLayer<Dtype>::Forward_cpu(const Dtype* bottom_data,
  ...
  for (int t = 0; t < T_; ++t) {
    ...
    caffe_mul<Dtype>(B_*H_, o_t, c_t, h_t);
  }
...
template <typename Dtype>
void LstmLinearLayer<Dtype>::Backward_cpu(const Dtype* top_data,
    Dtype* top_diff,
  ...
  for (int t = T_-1; t >= 0; --t) {
    ...
    caffe_mul<Dtype>(B_*H_, c_t, dh_t, do_t);
...
template <typename Dtype>
void LstmLinearLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
    vector<Blob<Dtype>*>* top) {
...
template <typename Dtype>
void LstmLinearLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
