  ...
  for (int layer_id = layers_.size() - 1; layer_id >= 0; --layer_id) {
    ...
    if (layer_need_backward_[layer_id]) {
      ...
      LOG(INFO) << layer_names_[layer_id]
                ...
                << " layer contributes loss: " << layer_contributes_loss;
    }
...
void Net<Dtype>::CopyTrainedLayersFrom(const NetParameter& param) {
  ...
  for (int i = 0; i < num_source_layers; ++i) {
    ...
    for (int j = 0; j < target_blobs.size(); ++j) {
      LOG(INFO) << "num t: " << target_blobs[j]->num() << " s: " << source_layer.blobs(j).num();
      LOG(INFO) << "channels t: " << target_blobs[j]->channels() << " s: " << source_layer.blobs(j).channels();
      LOG(INFO) << "height t: " << target_blobs[j]->height() << " s: " << source_layer.blobs(j).height();
      LOG(INFO) << "width t: " << target_blobs[j]->width() << " s: " << source_layer.blobs(j).width();
      ...
      /* Just for debugging
      if (source_layer_name == "target_score_pred") {
        float norm_c1 = 0.0, max_c1 = 0.0;
        float norm_c2 = 0.0, max_c2 = 0.0;
        float norm_v = 0.0, max_v = 0.0;
        const Dtype* blob_data = target_blobs[0]->cpu_data();
        for (int h = 0; h < 64; ++h) {
          for (int w = 0; w < 192; ++w) {
            float val = static_cast<float>(blob_data[h*192 + w]);
            if (w < 64) {
              norm_c1 += val*val;
              if (val > max_c1) {
                max_c1 = val;
        ...
        LOG(INFO) << "------------------> target_score_red <-----------------";
        LOG(INFO) << "norm_c1:" << (norm_c1/4096) << ", norm_c2:" << (norm_c2/4096)
                  << ", norm_v:" << (norm_v/4096);
        LOG(INFO) << "max_c1:" << max_c1 << ", max_c2:" << max_c2
                  << ", max_v:" << max_v;
        LOG(INFO) << "------------------> target_score_red <-----------------";
...
template <typename Dtype>
void Net<Dtype>::ToProto(NetParameter* param, bool write_diff, bool truncate_protos) {
  param->Clear();
  ...
  for (int i = 0; i < layers_.size(); ++i) {
    ...
    if (truncate_protos) {
      int truncate_num = 30;
      if (layers_[i]->layer_param().type() == LayerParameter_LayerType_INNER_PRODUCT) {
        truncate_num = 1;
      }
      DLOG(INFO) << "Layer " << layers_[i]->layer_param().name() << ", truncate_num: " << truncate_num;
      layers_[i]->ToTruncatedProto(layer_param, write_diff, truncate_num);
    } else {
